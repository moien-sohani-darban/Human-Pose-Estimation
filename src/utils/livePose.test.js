...
test("capture dimensions preserve aspect ratio and cap the longest side", () => {
  assert.deepEqual(computeInferenceDimensions(1920, 1080), {
    width: 960,
    height: 540,
  });
  assert.deepEqual(computeInferenceDimensions(720, 1280), {
    width: 540,
    height: 960,
  });
  assert.deepEqual(computeInferenceDimensions(640, 480), {
    width: 640,
    height: 480,
  });
});

test("camera capture reuses canvas context and bounded geometry", async () => {
  const draws = [];
  ...
  const first = await captureCameraFrame(video, canvas);
  const second = await captureCameraFrame(video, canvas, first.context);
...
test("JPEG capture returns bytes and enforces payload ceiling", async () => {
...
test("camera request uses video only and stop releases tracks", async () => {
  const tracks = [
    {
      stopped: 0,
      stop() {
  ...
  const returned = await requestCamera({
...
test("camera errors are normalized", () => {
  assert.equal(
    normalizeCameraError({ name: "NotAllowedError" }).code,
    "camera_permission_denied",
  );
  assert.equal(
    normalizeCameraError({ name: "NotFoundError" }).code,
    "camera_not_found",
  );
  assert.equal(
    normalizeCameraError({ name: "NotReadableError" }).code,
    "camera_in_use",
  );
});
...
test("scheduler never allows more than one inference in flight", async () => {
  const scheduled = [];
  const captures = [];
  const resolvers = [];
  const results = [];
  ...
  const scheduler = createLiveScheduler({
    captureFrame: async () => {
      const frame = captures.length + 1;
      captures.push(frame);
      return frame;
    },
    estimateFrame: (frame) =>
      new Promise((resolve) =>
        resolvers.push(() => resolve({ frame })),
      ),
