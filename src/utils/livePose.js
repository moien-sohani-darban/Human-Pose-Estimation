...
export function computeInferenceDimensions(width, height, maxDimension = LIVE_CAPTURE_MAX_DIMENSION) {
...
export async function requestCamera(mediaDevices, deviceId = "") {
  if (!mediaDevices || typeof mediaDevices.getUserMedia !== "function") {
    throw { code: "camera_unavailable", message: "Camera access is unavailable. Check Windows camera permissions and connect a camera, then try again." };
  }
...
export function normalizeCameraError(error) {
  ...
  if (name === "NotAllowedError" || name === "SecurityError") {
    return { code: "camera_permission_denied", message: "Camera permission was denied. Allow camera access and try again." };
  }
  if (name === "NotFoundError" || name === "DevicesNotFoundError") {
    return { code: "camera_not_found", message: "No usable camera was found. Connect or enable a camera, then try again." };
  }
  if (name === "NotReadableError" || name === "TrackStartError") {
    return { code: "camera_in_use", message: "The camera could not start. Close other applications using it, then try again." };
  }
  return { code: "camera_start_failed", message: "The camera could not be started. Check the device and try again." };
}

export function canvasToJpegBytes(canvas, quality = LIVE_CAPTURE_JPEG_QUALITY) {
  return new Promise((resolve, reject) => {
    canvas.toBlob(async (blob) => {
      if (!blob) {
        reject({ code: "frame_encode_failed", message: "The captured media frame could not be encoded. Stop analysis and try again." });
        return;
      }
      if (blob.size > MAX_FRAME_BYTES) {
        reject({ code: "frame_too_large", message: "The captured media frame exceeds the 4 MiB safety limit. Reduce the source resolution and try again." });
        return;
      }
...
export async function captureMediaFrame(
  ...
  if (
    ...
    throw { code: notReadyCode, message: notReadyMessage };
  }
